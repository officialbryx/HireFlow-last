...
def extract_skills(text):
    ...
    soft_skill_patterns = [
        ...
        r'\b\w+ation\b',  # Words ending in 'ation' often soft skills
...
def perform_resume_analysis(text):
    ...
    if analysis["personal_info"]["linkedin"]:
        print(f"   • LinkedIn: {analysis['personal_info']['linkedin']}")
...
def extract_personal_info(text):
    ...
    for pattern in website_patterns:
        ...
        filtered_websites = [w for w in websites if 'linkedin' not in w.lower()]
        if filtered_websites:
            personal_info["website"] = filtered_websites[0]
            break
