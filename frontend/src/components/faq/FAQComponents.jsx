...
import { MagnifyingGlassIcon, QuestionMarkCircleIcon, DocumentArrowDownIcon } from '@heroicons/react/24/outline';
  ...
  return (
    <div className="relative mt-4">
      ...
      {searchQuery && (
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5 text-gray-500 hover:text-gray-700">
    ...
    <div className="border border-gray-100 rounded-lg overflow-hidden shadow-sm bg-white">
      ...
      >
        <div className="flex items-center gap-2">
          <div className={`${isActive ? 'text-blue-600' : 'text-gray-500'} transition-colors`}>
            {SectionIcon ? <SectionIcon className="w-5 h-5" /> : null}
          </div>
          <h2 className={`font-semibold ${isActive ? 'text-blue-800' : 'text-gray-800'} transition-colors`}>
    ...
    <div className="border-t first:border-0 border-gray-100">
      ...
      <button
        ...
        className={`flex justify-between items-center w-full p-3 text-left rounded-md mt-2 ${
          isActive ? 'bg-gray-50 text-blue-700' : 'hover:bg-gray-50 text-gray-800'
        }`}
        aria-expanded={isActive}
...
export function EmptySearchResult() {
  return (
    <div className="text-center py-8">
      ...
      <p className="text-gray-400 mt-1">Try using different keywords or browsing the categories</p>
    </div>
  );
}
